...
foreach($employees as $empid => $emp){
    ...
    // ── Overtime (itemized) ───────────────────────────────
    $ot_query = $conn->query("SELECT date_overtime, hours, rate,
    ...
    if(!empty($overtimes)){
        foreach($overtimes as $ot){
            ...
            <tr>
                <td></td>
                <td></td>
                <td width="25%" align="right">OT ('.date('M d', strtotime($ot['date_overtime'])).' / '.$ot['hours'].'hrs):</td>
                <td width="25%" align="right">'.number_format($ot['ot_amount'], 2).'</td>
            </tr>';
        }
    }
    ...
    $contents .= '
            ...
            <tr>
                <td></td>
                <td></td>
                <td width="25%" align="right">Global Deduction:</td>
                <td width="25%" align="right">'.number_format($deduction, 2).'</td>
            </tr>';

    // Personal deduction rows (itemized)
